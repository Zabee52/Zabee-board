...
# 영철님의 힌트에 따르면 조합(nCm)을 활용하는 것 같다고 하신다.
# nCm : n에서 m개를 뽑는 경우의 수
# 위키에서 찾아보니 nCm을 구하는 공식은
# n! / ( (n-m)! * m! ) 이라고 한다. math 라이브러리 써서 한 번 풀어보자.
# 여기서는 m이 n보다 항상 크거나 같기 때문에 mCn이라고 표현하는게 맞겠다. 그러므로 m! / ( (m-n)! * n! )
